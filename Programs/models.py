...
class VoiceStat(models.Model):
    ...
    def __str__(self):
        if self.voice_stat_link is not None:
            temp1 = str(self.voice_stat_link[:20]) + ' ...'
        else:
            temp1 = None
        if self.voice_stat_type is not None:
            temp2 = str(self.voice_stat_type[:20]) + ' ...'
        else:
            temp2 = None
...
class VideoStat(models.Model):
    ...
    def __str__(self):
        if self.video_stat_link is not None:
            temp1 = str(self.video_stat_link[:20]) + ' ...'
        else:
            temp1 = None
        if self.video_stat_type is not None:
            temp2 = str(self.video_stat_type[:20]) + ' ...'
        else:
            temp2 = None
        return f'{temp1}   {temp2}'
...
class StreamType(models.Model):
    ...
    def __str__(self):
        if self.voice_content is not None and self.voice_content.voice_link is not None:
            temp1 = str(self.voice_content.voice_link[:20]) + ' ...'
        else:
            temp1 = None
        if self.video_content is not None and self.video_content.video_link is not None:
            temp2 = str(self.video_content.video_link[:20]) + ' ...'
        else:
            temp2 = None
...
class Program(models.Model):
    ...
    created_by = models.ForeignKey(User, on_delete=models.CASCADE, editable=False, verbose_name='ایجاد کننده', related_name='created')
